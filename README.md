...
| ------ | ------ | ------ | ------ | ------ |
| 阶段一 | `v0.1.x` | ~50 | 二（表设计/搭建）、三（登录模块）、四（基础秒杀） | 收尾中 |
...
- [ ] 4.7 JMeter 基线性能测试 + 阶段一总结 — `seckcpp0407`
- [ ] 4.8 应用层锁（`std::mutex` / 自旋 / 原子）优化重复下单 — `seckcpp0408`

> 当前 `/api/seckill` 已实现 4.3 / 4.6 核心逻辑（原子扣减 + 幂等），但博客文章 4.3 / 4.6 尚未写；4.4 / 4.5 为"先复现问题再解"的教学篇，代码侧超卖 / 重复下单已解决，文章仍待产出。
