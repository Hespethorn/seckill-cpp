...
| ------ | ------ | ------ | ------ | ------ |
| 阶段一 | `v0.1.x` | ≈341（实测基线） | 二（表设计/搭建）、三（登录模块）、四（基础秒杀） | 收尾中 |
| 阶段二 | `v0.2.x` | 数千 | 五（缓存层 Redis） | 待开始 |
| 阶段三 | `v0.3.x` | 数万 | 六（MQ 削峰填谷） | 待开始 |
| 阶段四 | `v1.0.0` | 30000+ | 七（Lua 原子预扣）、八（防刷限流） | 待开始 |

> 阶段一收尾清单（当前 4 项待做，已拆到对应博客章节）：
> 1. 登录 / 注册模块 → 第三章 3.1–3.2
> 2. 短信验证码 + 登录安全加固 → 第三章 3.3–3.6
> 3. JMeter 基线压测 → 第四章 4.4–4.7（**已实测：QPS≈341.3 / p95≈0.375s / 0 超卖**，基线数字落 README 第四章末 + 项目记忆；博客 4.4/4.5/4.7 待产出）
> 4. 应用层锁（`std::mutex` / 自旋 / 原子）优化重复下单 → 第四章 4.8
>
...
### 第三章 登录模块开发（阶段一 `v0.1.x`，自实现鉴权，无 SaToken）（待做）

- [ ] 3.1 整合 jwt-cpp + Redis 会话：用户注册接口 — `seckcpp0301`
- [ ] 3.2 用户登录接口（bcrypt / scrypt 密码哈希）— `seckcpp0302`
- [ ] 3.3 短信验证码接口（redis-plus-plus + 自实现线程池 / 协程）— `seckcpp0303`
...
- [ ] 3.7 退出登录接口 — `seckcpp0307`
- [ ] 3.8 代码细节重构 — `seckcpp0308`
- [ ] 3.9 滑块验证码（自实现或接入行为验证）— `seckcpp0309`

> 注：登录模块在阶段一里走 `jwt-cpp + Redis 会话`，即 **Redis 提前到 `v0.1.x` 进场**（原架构演进中 Redis 属阶段二）。实现 3.1 时需先引入 `redis-plus-plus` 依赖。
...
- [x] 4.7 基线性能测试（curl harness：QPS≈341 / p95≈0.375s / 0 超卖）+ 阶段一总结（博客待产出）— `seckcpp0407`
- [ ] 4.8 应用层锁（`std::mutex` / 自旋 / 原子）优化重复下单 — `seckcpp0408`

> 当前 `/api/seckill` 已实现 4.3 / 4.6 核心逻辑（原子扣减 + 幂等），但博客文章 4.3 / 4.6 / 4.4 / 4.5 / 4.7 尚未写；代码侧超卖 / 重复下单已解决并实测验证（correct 模式 0 超卖、baseline QPS≈341），文章按"项目先行、博客随后"约定待产出。

#### 阶段一基线（实测，2026-09-02，WSL i7-14650HX + 本地 MySQL）

| 项 | 数值 |
| --- | --- |
| 工具 | `scripts/jmeter-baseline.sh` 默认 curl 并发 harness（apt jmeter 在 WSL 不可用） |
| 并发 / 时长 | 100 并发 / 60s，库存 1e8（不售罄，专测"卖出"事务路径） |
| **QPS** | **≈ 341.3**（MySQL 端 sold=20492 → 真实 ≈341.5） |
| avg / p50 / p95 / p99 延迟 | 0.2888s / 0.2736s / 0.3752s / 0.4847s |
| 200 / 409 / 失败 | 20479 / 0 / 0（harness 计数）；MySQL sold=orders=20492（0 超卖） |
| 不超卖核对 | correct 模式 100 抢 10 → 10×200 + 90×409，sold=orders=10 |
